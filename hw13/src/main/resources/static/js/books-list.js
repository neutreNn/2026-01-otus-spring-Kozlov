(() => {
    const page = document.querySelector("[data-books-page]");
    ...
    const createActions = (book) => {
        ...
        actions.append(createLink(`/books/${book.id}`, "Открыть", "button button--ghost"));
        if (canEdit) {
            actions.append(createLink(`/books/${book.id}/edit`, "Редактировать", "button button--ghost"));
        }
        if (canDelete) {
            actions.append(createDeleteButton(book));
        }
        return actions;
    };
